...
#-------------------------------------------------------------------------
    ...
    def untag_obs(self):
        for agent in self.world.obstacles:
            agent.tagged = False



    def obstacle_avoidance(self):
        boxlength = self.min_box_length + (self.speed()/self.max_speed)*self.min_box_length
        self.tag_obs(boxlength)
        distanceToClosestObs = 99999999999
        ObsPosition = Vector2D()
        Obstacle_Obj = None

        for obs in self.world.obstacles:
            if obs.tagged is True:
                if obs.pos.x >= 0:
                    expandedRad = obs.radius + self.bRadius

                    if obs.pos.y < expandedRad:
                        cx = obs.pos.x
                        cy = obs.pos.y

                        sqrt = math.sqrt(expandedRad*expandedRad - cy*cy)

                        intersection_pt = cx - sqrt
                        if intersection_pt <= 0:
                            intersection_pt = cx + sqrt

                        if intersection_pt < distanceToClosestObs:
                            distanceToClosestObs = intersection_pt
                            ObsPosition = obs.pos
                            Obstacle_Obj = obs

        steering_force = Vector2D()
        if Obstacle_Obj is not None:
            multiplier = 1.0 + (self.min_box_length - ObsPosition.x) / self.min_box_length
            steering_force.y = (Obstacle_Obj.radius - obsPosition.y) * multiplier

            braking_force = 0.2
            steering_force.x = (Obstacle_Obj.radius - obsPosition.x) * braking_force
